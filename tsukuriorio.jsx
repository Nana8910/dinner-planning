  ...
  useEffect(() => {
    (async () => {
      ...
      if (saved && saved.dishes) {
        ...
        // 中身のある週を表示する（空の今週に着地して「何も無い」を防ぐ）。
        // 優先: 予定済みの最新週 → ストックがある最新週 → メニューの最新週 → 今週
        const dated = payload.dishes.filter((d) => d.date).map((d) => d.date).sort();
        const stockWeeks = payload.dishes
          .map((d) => d.weekId)
          .filter(Boolean)
          .map((id) => weekStartISO(id))
          .sort();
        let target;
        if (dated.length) target = mondayISO(dated[dated.length - 1]);
        else if (stockWeeks.length) target = mondayISO(stockWeeks[stockWeeks.length - 1]);
        else if (weeks.length) target = weekStartISO(pickCurrentWeek(weeks).id);
        else target = mondayISO();
        setViewMonday(target);
      } else {
      ...
      <main className="om-main">
        {tab === "plan" && (
          <PlanView
            ...
            latestMenuMonday={menu.weeks.length ? weekStartISO(pickCurrentWeek(menu.weeks).id) : null}
            onPrev={() => setViewMonday(addDaysISO(viewMonday, -7))}
            onNext={() => setViewMonday(addDaysISO(viewMonday, 7))}
            onToday={() => setViewMonday(mondayISO())}
            onGoLatestMenu={() => setViewMonday(weekStartISO(pickCurrentWeek(menu.weeks).id))}
            onImport={importWeek}
            onAuto={() => {
  ...
  return (
    <div className="om-plan">
      ...
      <section className="om-shelf">
        ...
        {stock.length === 0 ? (
          <>
            ...
            {importedCount === 0 && !canImport && latestMenuMonday && latestMenuMonday !== viewMonday && (
              <button className="om-primary wide" onClick={onGoLatestMenu}>
                <CalendarDays size={16} /> ツクリオ最新メニュー（{weekLabelFromMonday(latestMenuMonday)}）へ
              </button>
            )}
          </>
        ) : (
          <div className="om-chips">
            {stock.map((d) => (
              <button
                key={d.id}
                className={"om-chip " + (d.source === "tsukurioki" ? "src-tsuk" : "src-home")}
                onClick={() => onOpenSheet(d.id, "assign")}
              >
                <DishDot d={d} />
                <span className="om-chip-name">{d.name}</span>
                {d.freezable ? (
                  <Snowflake size={13} className="om-store-i" />
                ) : (
                  <Refrigerator size={13} className="om-store-i" />
                )}
              </button>
            ))}
          </div>
      ...
      <div className="om-legend">
        <span><i className="lg-tsuk" />ツクリオ</span>
        <span><i className="lg-home" />自作</span>
        <span className="om-legend-note">冷蔵／冷凍はアイコンで表示</span>
      </div>
...
function DishDot({ d }) {
  const tsuk = d.source === "tsukurioki";
  return (
    <span
      className="om-dot"
      style={{ background: tsuk ? "var(--tsuk)" : "var(--home)" }}
      title={tsuk ? "ツクリオ" : "自作"}
    />
  );
}
...
:root{
  ...
  /* おかずの出どころ色：ツクリオ＝ブランドのオレンジ／自作＝グリーン（テーマに関係なく固定） */
  --tsuk:#D6852F; --tsuk-bg:#FBEEDC; --home:#3F8F57; --home-bg:#E7F1E7;
...
.om-chip.src-tsuk{background:var(--tsuk-bg); border-color:#EAD6B5;}
.om-chip.src-home{background:var(--home-bg); border-color:#CADEC9;}
.om-chip:hover{filter:brightness(.97);}
.om-chip-name{max-width:160px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;}
.om-store-i{color:var(--ink2); flex-shrink:0;}
.om-frz{color:var(--frozen);} .om-frd{color:var(--fresh);}
.om-dot{width:8px;height:8px;border-radius:50%; flex-shrink:0;}

/* 出どころの凡例 */
.om-legend{display:flex; align-items:center; gap:14px; justify-content:center; margin:2px 0 4px; font-size:11px; font-weight:700; color:var(--ink2);}
.om-legend span{display:inline-flex; align-items:center; gap:5px;}
.om-legend i{width:10px; height:10px; border-radius:50%; display:inline-block;}
.om-legend .lg-tsuk{background:var(--tsuk);}
.om-legend .lg-home{background:var(--home);}
.om-legend-note{color:#A7A296; font-weight:600;}
...
/* planned dish — 色は出どころ（ツクリオ/自作）、冷蔵冷凍はアイコンのみ */
.om-pd{border-radius:10px; overflow:hidden; border:1px solid transparent;}
.om-pd.src-tsuk{background:var(--tsuk-bg);}
.om-pd.src-home{background:var(--home-bg);}
...
.om-pd-badge{display:inline-flex; align-items:center; gap:3px; font-size:9.5px; font-weight:800;
  padding:2px 5px; border-radius:6px; flex-shrink:0; color:#fff;}
.om-pd.src-tsuk .om-pd-badge{background:var(--tsuk);}
...
.theme-heisei .om-chip.src-tsuk{background:var(--tsuk-bg); border-color:#EAD6B5;}
.theme-heisei .om-chip.src-home{background:var(--home-bg); border-color:#CADEC9;}
