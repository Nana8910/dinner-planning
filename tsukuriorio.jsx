...
const initialSettings = { people: 2, days: 7, freshDays: 3, startDate: todayISO() };
  ...
  useEffect(() => {
    (async () => {
      ...
      if (saved && saved.dishes) {
        ...
        // 初回起動：次回お届け週（週3食ぶん）を読み込んで、すぐデータが見える状態に
        const seed = WEEKS[0].dishes
          .filter((d) => d.plan !== "5食")
    ...
    <div className="om-dishes">
      ...
      <section className="om-card">
        ...
        <button className="om-primary wide" onClick={loadWeek}>
          <Plus size={17} /> この週のおかずを追加（{preview.length}品）
        </button>
